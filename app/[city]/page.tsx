  ...
  return (
    <>
      ...
      <main className="flex flex-wrap gap-4 justify-center p-3">
        {cityWeather?.forecast?.forecastday[0]?.hour?.map(
          (item: any, i: number) => (
            <div
              className="flex border shadow-lg rounded-2xl flex-col items-center gap-2 p-2 w-60"
              key={i}
            >
              <h1>{item.time}</h1>
      ...
      <button
        className="bg-yellow-800 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded fixed bottom-3 right-3"
        onClick={() => {
          router.push("/");
        }}
      >
